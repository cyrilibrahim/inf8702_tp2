...
vec4 lightSpec(in int i, in vec3 normal, in vec3 halfVector, in float shininess)
{
	...
	//Pas d'attenuation pour la lumière directionnelle
	float attenuation = 1.0;
	if (l.Position.w == 0) {
		vec3 VP = l.Position.xyz - VertexPosition_cameraSpace;
		float d = length(VP);
		attenuation = 1/(l.Attenuation[0] + l.Attenuation[1]*d + l.Attenuation[2]*d*d);
	}

	//halfVector est raisonnablement normalisé
	float nDotH = max(dot(normal,halfVector),0);

	return vec4(pow(nDotH,shininess)*l.Specular*attenuation,0.0);
}
...
{   
	...
	{	
		...
		if (perlinOn == 1) {
			// Normales précalculée en espace tangent, avec bruit
			normal = noise;
		} else {
